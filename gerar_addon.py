...
def buscar_arquivos_do_proxy():
    ...
    print(f"🔗 Conectando a: {url_pasta}")
    
    # 1. Tentativa via API POST (Padrão Cloudflare Worker Index)
    try:
    ...
    # 2. Tentativa via GET na rota de pasta
    try:
        ...
        if res.status_code == 200:
            ...
            # Se for resposta HTML, faz o parse dos links diretamente
            soup = BeautifulSoup(res.text, "html.parser")
            arquivos = []
            for a in soup.find_all("a", href=True):
                href = a["href"]
                texto = a.get_text().strip()
                if "/file/" in href or any(texto.lower().endswith(ext) for ext in ['.mkv', '.mp4', '.avi']):
...
def processar_filmes():
    ...
    if not arquivos:
        print("❌ ERRO: Nenhum arquivo foi retornado do Proxy. Verifique as permissões do Worker.")
        return
    ...
    for item in arquivos:
        ...
        # Filtra apenas extensões de vídeo conhecidas
        if not nome_arq or not any(nome_arq.lower().endswith(ext) for ext in ['.mkv', '.mp4', '.avi', '.webm']):
            continue
            
        stream_url = f"{PROXY_BASE_URL}/file/{file_id}" if not file_id.startswith("http") else file_id
        ...
        if info_tmdb:
            ...
            print(f"✅ Mapeado com sucesso: {info_tmdb.get('title')}")
        else:
    ...
    if metas:
        ...
        print(f"\n🎉 Sucesso! {len(metas)} filmes foram salvos no catalog.json!")
